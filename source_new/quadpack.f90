...
!! This source file was modified by Marco Raveri.
...
contains
    ...
    !! @todo      Can't this work without nmess? Can we replace this with a better error handler?
    !!
    subroutine xerror ( xmess, nmess, nerr, level )

        implicit none

        integer   ( kind = 4 ), intent(in) :: level !< input error level (?)
        integer   ( kind = 4 ), intent(in) :: nerr  !< input number of the error (?)
        integer   ( kind = 4 ), intent(in) :: nmess !< input length of the error message (?)
        character ( len = *  ), intent(in) :: xmess !< input error message
    ...
    !!            11 September 2015
    !! @copyright GNU Public License v3
    !! @todo      Can we remove the horrible gotos without altering functionality and performances? \n
    ...
    function dqwgts ( x, a, b, alfa, beta, integr )
        ...
        real    ( kind = 8 ), intent(in) :: x      !< x
        real    ( kind = 8 ), intent(in) :: a      !< a
        real    ( kind = 8 ), intent(in) :: b      !< b
        real    ( kind = 8 ), intent(in) :: alfa   !< alfa
        real    ( kind = 8 ), intent(in) :: beta   !< beta
        integer ( kind = 4 ), intent(in) :: integr !< integr
